...
while(t_act<t_end)
    ...
    for i=2:nx-1
        ...
        W1L =  4* sqrt(par.beta*sn/(2*par.rho*An)) *(AL^.25-An^.25) + vL;
        W2R = -4* sqrt(par.beta*sn/(2*par.rho*An)) *(AR^.25-An^.25) + vR;
...
plot(x,v+a,'r');
plot(x,v-a,'r');
ylabel('velocity and PWV [m/s]');
subplot(4,1,3);
plot(x,A);
hold on;
plot(x,Anx,'k');
ylabel('cross-section area [m2]');
subplot(4,1,4);
plot(x,q);
xlabel('x [m]');
