...
int main(int argc, char* argv[])
{
   ...
   vector<double> wall_par(6,1.); // brain, face, hands, legs, spine, coronary
   vector<double> elas_par(6,1.); // brain, face, hands, legs, spine, coronary
   ...
   if(argc == n_par)
   {
      ...
      for(int i=0; i<5; i++)
      {
         perif_par[i] = stod(argv[i+11],0);
      }
      for(int i=0; i<6; i++)
      {
         len_par[i] = stod(argv[i+16],0);
      }
      for(int i=0; i<6; i++)
      {
         diam_par[i] = stod(argv[i+22],0);
      }
      for(int i=0; i<6; i++)
      {
         node_res_par[i] = stod(argv[i+28],0);
      }
      for(int i=0; i<6; i++)
      {
         wall_par[i] = stod(argv[i+34],0);
      }
      for(int i=0; i<6; i++)
      {
         elas_par[i] = stod(argv[i+40],0);
      }
   }
   else if(argc == 1)
   {
   ...
   fb->material_type = 0; // linear model
   ...
   for(int i=0; i<id_edge.size(); i++)
   {
      for(int j=0; j<id_edge[i].size(); j++)
      {
         ...
         fb->moc[0]->edges[idx]->nominal_thickness_start *= wall_par[i];
         fb->moc[0]->edges[idx]->nominal_thickness_end *= wall_par[i];
         fb->moc[0]->edges[idx]->elasticity *= elas_par[i];
      }
   }
