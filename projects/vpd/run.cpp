...
int main(int argc, char* argv[])
{
   ...
   if(argc != 2)
   {
      cout << "Wrong number of inputs: " << argc-1 << "  Correct: 1" << endl;
      return 0;
   }
   else
   {
      idx_cons = atoi(argv[1]);
   }

   // loading which VP to run
   ...
   ifstream file_in;
   string file_name = "vpd.csv";
   file_in.open(file_name);
   string line;
   if(file_in.is_open())
   {
      bool got_it = false;
      getline(file_in,line); // skipping header line
      while(getline(file_in,line) && !got_it)
      {
         // clearing spaces and \n
         ...
         int idx_file = stoi(sv[0],0);
         if(idx_file == idx_cons)
         {
            sex = sv[1];
            age = stoi(sv[2],0);
            res_mitral = stod(sv[3],0);
            res_aorta = stod(sv[4],0);
            pres_atrium = stod(sv[5],0);
            for(int i=0; i<4; i++)
            {
               res_node[i]    = stod(sv[6+i],0);
               res_perif_1[i] = stod(sv[10+i],0);
               res_perif_2[i] = stod(sv[10+i],0);
               cap_perif[i]   = 1./stod(sv[10+i],0); // to keep R*C const
               length[i]      = stod(sv[14+i],0);
               diameter[i]    = stod(sv[18+i],0);
               thickness[i]   = stod(sv[22+i],0);
               elas_spring[i] = stod(sv[26+i],0);
               elas_voigt[i]  = stod(sv[30+i],0);
            }
   ...
   string out_file_name = to_string(idx_cons) + "_" + sex + "_" + to_string(age);
   ...
   vector<double> fitness_function(11,0.);
   if(is_run_ok)
   {
      // saving stuff to file
      fb->save_results(save_dt,out_file_name);
      //fb->save_results(save_dt,out_file_name,model_name,model_type,el,nl);
      ...
      fitness_function[6] = average(fb->moc[0]->edges[A1idx]->volume_flow_rate_start,fb->moc[0]->edges[A1idx]->time)*1.e6*60.;
      ...
      for(int i=0; i<id_pwv.size(); i++)
      {
         ...
         for(int j=0; j<id_pwv[i].size(); j++)
         {
            ...
            lsum += l;
            fitness_function[7+i] += (vs+as + ve+ae)*.5 * l;
         }
         fitness_function[7+i] /= lsum;
      }
   }
