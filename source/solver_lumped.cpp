...
void solver_lumped::initialization(double time_period)
{
	...
	period_.assign(time_upstream.size(),0.);
...
void solver_lumped::coefficients_newton(double t_act)
{
	...
	for(int i=0; i<number_of_edges; i++)
	{
		...
		else if(edges[i]->type_code == 12) // piecevise constant resistor, models compression
		{
...
void solver_lumped::set_non_SI_parameters()
{
	...
	for(int i=0; i<number_of_edges; i++)
	{
		...
		else if(edges[i]->type_code == 12) // piecewise constant resistance
		{
			edges[i]->par_non_SI.push_back(edges[i]->parameter[0]/mmHg_to_Pa*1.e-6);
			edges[i]->par_non_SI.push_back(edges[i]->parameter[1]/mmHg_to_Pa*1.e-6);
		}
...
double solver_lumped::get_interp_val(int index, double t_act, int up_b){
	...
	while(!got_it)
	{
		...
		double t = t_act-period_[up_idx]*(time_upstream[up_idx].back()-time_upstream[up_idx][0]);
	...
	double t_in = t_act-period_[up_idx]*time_upstream[up_idx].back(); // actual time of the simulation
...
vector<double> solver_lumped::p_drop_q(double q, int index) {
    ...
    vector<double> qv(edges[index]->qv_c.size());
    vector<double> dp(edges[index]->dp_c.size());
